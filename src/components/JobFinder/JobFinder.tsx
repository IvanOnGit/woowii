...
export default function JobFinder() {
  ...
  useEffect(() => {
    const fetchMatchNotifications = async () => {
      ...
      try {
        const response = await fetch(`https://api.woowiihr.com/api/auth/user-matches?userId=${userId}`);
        const data = await response.json();
  ...
  const markNotificationsAsViewed = async () => {
    ...
    try {
      ...
      await fetch('https://api.woowiihr.com/api/auth/mark-notifications-viewed', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const response = await fetch('https://api.woowiihr.com/api/auth/jobs');
        const data = await response.json();
        setAllJobs(data);
      } catch (error) {
